...
**Real-time run (2026-09-17, `scripts/realtime_backtest.py`, `docs/realtime_trigger_backtest.md`; 163 quarter starts 1986Q1-2026Q3; default config: envelope q0.9, r > g required, 4-quarter persistence).** Hindsight table on the 2026-07-01 vintage: 0 triggered quarters. Real-time table: 41 triggered as-of dates, all between 1986 and 1997, none since; the two tables disagree on 25% of dates. Every disagreement is the envelope, not a data revision: in real time the 90th-percentile primary balance was a deficit (-0.7% to 0% of GDP) until the late-1990s surpluses entered the sample, so a pb* of 1-2.5% of GDP (r above g by 2-3 points in the 1980s) breached it for a decade; in hindsight the envelope is 2.2% and nothing breaches for four consecutive quarters. Two consequences. (i) The "which quarters would have flagged" table is dominated by the benchmark's history-dependence, so the vintage problem is really a benchmark problem: a quantile of past outcomes measures what governments did, not what they could do, and before 1998 the US had never shown it could run a surplus. The reaction-function benchmark (Bohn / Ghosh et al., already a sensitivity in 0006) estimates a response rather than an envelope and does not have this pathology by construction. (ii) The identity trigger produced a ten-year real-time alarm (1986-1996) that never became a crisis; the ratio stabilized through growth and the 1990s consolidation. That is a documented case of "reachable but did not fire" (0006 review) and belongs in the thesis as evidence, not as an embarrassment. Two housekeeping notes: the 2026-09-15 methodology note said the default definition flags 2009Q2-Q3, while this run's hindsight table flags nothing, to reconcile (config or vintage difference); and the disagreement table repeats 1991-07-01 because two as-of dates saw the same last quarter when the reporting lag moved from 1 to 2 quarters at the first true GDP vintage (1991-12), so pre-1992 rows are also optimistic on timeliness.

**Benchmark comparison in real time (same day; `docs/realtime_trigger_backtest{,_reaction,_min,_actual}.md`).** Same trigger (r > g, 4 quarters, 4-quarter growth smoothing), four definitions of the feasible balance, share of aligned quarters triggered:

| benchmark | real time | hindsight | agreement | real-time episodes |
|---|---|---|---|---|
| envelope (q0.9 of past pb) | 25% | 0% | 74.8% | 1986-1994, 1996-1997; says "never" today |
| reaction function (Bohn/Ghosh, re-fit per date, 40-quarter minimum, so from 1991) | 31% | 32% | 99.3% | 1991-94, 1996-2000, 2002-03, 2009-10 |
| min(envelope, reaction) | identical to reaction row for row | | | |
| actual stance (trailing 4-quarter pb; "explosive under current policy", Bohn's test in trigger form) | 26% | 27% | 99.4% | 1986-1994, 1996Q2-Q3, 2009Q3-2010Q3 |

Reading: the envelope under-flags and is vintage-fragile (its level is set by the best past quarter, which arrived in 2000). The reaction function is vintage-robust but over-flags: it fires through the 1996-2000 surpluses, when the ratio was falling, because a fitted "typical response" underpredicts an atypically good one. The actual-stance benchmark is vintage-robust over the whole window, fires only in the two episodes a historian would name, and both ended without a crisis, which is the reachable-vs-fires distinction in the data. Recommendation: `actual` as the default trigger benchmark, `reaction` kept for what it is good at (the debt limit, where no observed response stabilizes the ratio), `envelope` as a sensitivity. Note before adopting: `evaluate_paths` (the simulator side) does not yet have the `actual` branch; on simulated paths it would read "pb* above the path's own trailing simulated pb", which is the natural definition and a small change. The single disagreement under both robust benchmarks is one quarter of timing at the 2008 onset (real time fires at 2008Q4, hindsight at Q3).
...
**Status update (2026-09-21): item (2) built and run, item (1) partially done, item (3) partially done.** `MacroVAR.credit_channel_transmission` (macro.py) and `VARBlock(transmission="credit", credit_elasticity=...)` (macroblock.py) add the second transmission option, sourced directly from Arellano, Bai & Bocola (2017) rather than Bocola (2016) alone: central -0.006 pp growth per 100bp premium (their estimate that sovereign risk explains ~30% of Italy's 2012 output decline against the period's average spread rise), band -0.002 to -0.010 from their 10%-50% attribution range, an Okun's-law step to unemployment flagged in the docstring as not itself literature-sourced. Recorded as P-20 in `PARAMETERS.md` and in `0005-quarterly-var-plus-premium.md`. The side-by-side comparison (item 2's full ask) is now run: `scripts/premium_transmission_comparison.py` / `docs/premium_transmission_comparison_findings.md`, real fitted VAR, identical draws, two P-01 slopes. Headline result: the channels' *instantaneous* coefficients differ ~94x on real data (var's g_nom coefficient -0.56 per 100bp of premium change vs credit-central's -0.006), and the synthetic test fixture agrees in this ordering (its hand-built coefficient is -0.386, same sign, same order of magnitude, also far larger than credit's), so `rate_transmission` is dramatically harsher *instantaneously* on both. But the *simulated* 30-year outcomes stay close (terminal debt/GDP within 0.36pp of GDP at the default slope, ~2.0pp at 2.5x the slope) because `rate_transmission` acts on the small quarterly premium change, not its level, and the VAR's own mean reversion unwinds most of each shock before it compounds — provisional evidence the mechanical debt-service channel, not either growth-transmission channel, is what drives this model's trajectories. Item (1), reframing `rate_transmission` as a stated monetary-tightening assumption rather than empirical loop content, is done in the 0005 doc's prose but not yet in a paper draft (none exists yet to reword). Item (3), stating the VAR-vs-state transmission-channel gap explicitly, is noted in the 0005 doc but the actual VAR-vs-state quantitative comparison has not been run (only VAR-var vs VAR-credit was). Not yet done: the kinked/threshold premium form (P-02) through this same comparison, which concentrates larger quarterly premium changes at high debt/GDP and is where channel choice would plausibly matter most; and the VAR-vs-state-block comparison itself. `tests/test_credit_channel.py` covers the new method's shape, sign, band ordering, and end-to-end behavioral difference on a synthetic fixture only; it deliberately does not assert a numeric target for how much of the instantaneous gap survives into the simulated outcome, since that is a property of the premium path and mean-reversion dynamics that the real side-by-side comparison, not a single synthetic fixture, should answer.
